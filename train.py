    ...
    with tf.GradientTape() as query_tape:
        ...
        for one_task in batch_task:
            ...
            for inner_step in range(inner_train_step):
                ...
                for i in range(len(inner_model.layers)):
                    if "max_pooling2d" in inner_model.layers[i].name or "flatten" in inner_model.layers[i].name:
                        continue

                    # kernel
                    inner_model.layers[i].trainable_variables[0].assign_sub(tf.multiply(lr_inner, inner_grads[k]))
                    # bias
                    inner_model.layers[i].trainable_variables[1].assign_sub(tf.multiply(lr_inner, inner_grads[k + 1]))
                    k += 2
